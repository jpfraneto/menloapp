...
<div data-device="mac"><p>Menlo uses your Mac and Xcode to build this app for your intended iPhone.</p><p class="ml-muted">You’ll need Xcode, an Apple Account for signing, and your iPhone. Menlo asks you to review the source before building.</p><div class="ml-sheet-actions"><a class="ml-button ml-open-app" href="${escape(app.open_url)}">Open in Menlo</a><a class="ml-button ml-button-secondary" href="/download/macos">Set up Menlo</a></div><p class="ml-small">First time? Install Menlo, open it from Applications, then return here and choose Open in Menlo.</p></div>
...
export function renderAppDirectory(baseUrl: string, directory: AppDirectory) {
  ...
  return menloPage("Discover apps", "Discover apps shared by their makers. Install them with Menlo on your Mac.", baseUrl, `<main id="main" class="ml-container ml-discovery"><div class="ml-discovery-heading"><h1>Apps, person to person.</h1><p>Discover apps shared by their makers.<br>Install them with Menlo on your Mac.</p></div><section aria-label="Discover apps"><div class="ml-install-note"><svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.4" aria-hidden="true"><rect x="3" y="3" width="14" height="10" rx="1"/><path d="M1 16h18M8 13v3m4-3v3"/></svg><span>Installing an app requires a Mac, Xcode, and your iPhone.</span></div>${directory.listings_unavailable ? `<p class="ml-feed-notice" role="status">Some app details are temporarily unavailable. <a href="/apps">Try again</a></p>` : ""}<div class="ml-app-grid">${directory.apps.length ? directory.apps.map(appCard).join("") : `<div class="ml-empty"><h2>The first apps will appear here.</h2><p>Have something to share? Turn your public GitHub app into a link.</p><a class="ml-button" href="#deploy" data-open-sheet="deploy">Share your app</a></div>`}</div></section></main>`);
}
