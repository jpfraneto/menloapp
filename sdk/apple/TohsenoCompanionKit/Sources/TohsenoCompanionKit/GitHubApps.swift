...
public struct GitHubAppStatus: Codable, Equatable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case slug, repository, commit
        case repositoryID = "repository_id"
        case installedCommit = "installed_commit"
    ...
    public func validate() throws {
        guard slug.count >= 2, slug.count <= 64,
              ...
              [commit, installedCommit, headCommit].compactMap({ $0 }).allSatisfy({ $0.range(of: #"^[a-f0-9]{40}$"#, options: .regularExpression) != nil })
        else { throw TohsenoCompanionError.invalidEncoding("Invalid GitHub app status") }
        if let checkedAt { _ = try CompanionTimestamp.parse(checkedAt) }
    }
}
