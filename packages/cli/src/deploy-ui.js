...
function ask(question) {
  return new Promise((resolve, reject) => {
    ...
    let answered = false;
    reader.on("close", () => { if (!answered) reject(new Error("Deploy cancelled. Run menloapp deploy when you are ready.")); });
...
export async function choose(ui, title, values, flag) {
  ...
  if (!ui.interactive) throw new Error(`${title}\n${choices}\n\nRun menloapp deploy in a terminal to choose, or use ${flag} ${shellQuote(values[0])}.`);
  ui.log(`\n${title}\n${choices}`);
  while (true) {
